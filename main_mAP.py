...
#        result_line = ("{}".format(ground_truth_img[0]),"{}".format(class_name),"{0:.2f}".format(ovmax),"{0:.2f}".format(float(prediction["confidence"])))
        result_line = (ground_truth_img[0],class_name,int(ovmax*100)/100,float(prediction["confidence"]))
...
#        cv2.waitKey(0) # show for 20 ms
    ...
    with open('result_detail.txt', 'wt') as f:
        for line_each in lines_sorted:
            f.write('{},{},{},{}\n'.format(line_each[0],line_each[1],line_each[2],line_each[3]))
